        ...
        <div className="max-w-7xl mx-auto px-6">
          {isLoading ? (
            ...
            <div className={`${viewMode === 'grid' ? 'grid lg:grid-cols-3 md:grid-cols-2 gap-8' : 'space-y-6'}`}>
              ...
              <Card
                key={project.id}
                className={`group bg-white/85 backdrop-blur-sm border-teal-200/50 shadow-lg hover:shadow-2xl transition-all duration-700 transform hover:-translate-y-4 cursor-pointer overflow-hidden ${
                  project.featured ? 'ring-2 ring-teal-400/50' : ''
                } ${viewMode === 'list' ? 'flex' : ''}`}
                style={{
                  transform: `perspective(1000px) rotateX(${mousePosition.y * 0.015}deg) rotateY(${mousePosition.x * 0.015}deg) translateY(-16px)`,
                  transitionDelay: `${index * 100}ms`
                }}
              ...
              >
                {/* Project Image */}
                <div className={`relative overflow-hidden bg-gradient-to-br from-teal-100 to-cyan-100 ${
                  ...
                  {/* Hover Overlay */}
                  <div className={`absolute inset-0 bg-gradient-to-t from-teal-900/70 to-transparent transition-opacity duration-300 ${
                    hoveredProject === project.id ? 'opacity-100' : 'opacity-0'
                  }`}>
                    {/* Category Badge */}
                    <div className="absolute top-4 left-4">
                      <div className="bg-white/90 backdrop-blur-sm rounded-full px-3 py-1">
                        <span className="text-teal-900 text-xs font-medium">{project.category || 'Web Development'}</span>
                      </div>
                    </div>
                    ...
                    <div className="absolute bottom-4 right-4 flex space-x-2">
                      {project.url && project.url !== '#' && (
                        <a
                          ...
                          className="w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg transform hover:scale-110 transition-all duration-300"
                          onClick={(e) => {
                            e.stopPropagation();
                ...
                {/* Project Content */}
                <CardContent className={`relative ${viewMode === 'list' ? 'flex-1' : ''} p-6`}>
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="text-xl font-bold text-teal-900 group-hover:text-teal-800 transition-colors">
                      {project.title}
                    </h3>
                  ...
                  <div className="flex items-center space-x-4 text-sm text-teal-600 mb-3">
                    <div className="flex items-center space-x-1">
                  ...
                  <p className="text-teal-700 leading-relaxed mb-4 text-sm">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {project.technologies.slice(0, 4).map((tech, techIndex) => (
                      <Badge
                        key={techIndex}
                        variant="outline"
                        className="border-teal-300 text-teal-700 hover:bg-teal-100 transition-colors text-xs"
                      >
                        {tech}
                      </Badge>
                    ))}
                  ...
                  {selectedProject === project.id && (
                    <div className="mt-4 p-4 bg-teal-50/50 rounded-xl border border-teal-200/30">
                      <div className="grid md:grid-cols-2 gap-4 mb-4">
                        <div>
                          <h4 className="font-semibold text-teal-800 mb-1">Project Details:</h4>
                        ...
                        <div>
                          <h4 className="font-semibold text-teal-800 mb-1">All Technologies:</h4>
                          <div className="flex flex-wrap gap-1">
                      ...
                      {project.testimonial && (
                        <div className="bg-white/50 rounded-lg p-4">
                          <h4 className="font-semibold text-teal-800 mb-2">Client Testimonial:</h4>
                          <blockquote className="text-teal-700 italic text-sm mb-2">
                  ...
                  {/* Action Button */}
                  <Button 
                    className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white rounded-xl transform hover:scale-105 transition-all duration-300 shadow-md text-sm py-2"
                  >
